...
std::ranges::view auto Selection(R&& population, const int n_parents)
{
    std::ranges::sort(population, [](const auto& a, const auto& b) { return a.score < b.score; });
...
template <Solution T, StopConditionFunctor S, class P>
class GeneticAlgorithmSolver
{
public:
    explicit GeneticAlgorithmSolver(S stop_condition) : stop_condition(stop_condition) {}
...
public:
        ...
        const float mutation_rate)
    {
        ...
        auto p_best = *std::ranges::min_element(population, [](const auto& a, const auto& b) { return a.score < b.score; });
        while (StopCondition(stop_condition, p_best.score)) {
            ...
            auto q_best = *std::ranges::min_element(population, [](const auto& a, const auto& b)
            {
