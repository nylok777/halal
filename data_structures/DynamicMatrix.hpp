...
public:
    ...
    const std::vector<const T*> Column(const int n_col) const;
    const std::vector<std::vector<const T*>> Columns() const;
...
public:
    ...
    public:
        ...
        friend row_iterator DynamicMatrix::begin() const;
        friend row_iterator DynamicMatrix::end() const;
...
template <typename T>
DynamicMatrix<T>::row_const_iterator::row_const_iterator(const std::vector<T>* const end_row,
...
template <typename T>
const T& DynamicMatrix<T>::row_const_iterator::operator*()
{
    return *current_vec_iter;
}

template <typename T>
typename DynamicMatrix<T>::row_const_iterator DynamicMatrix<T>::row_const_iterator::operator++()
{
    ++current_vec_iter;
...
template <typename T>
typename DynamicMatrix<T>::row_const_iterator DynamicMatrix<T>::begin() const
{
    return row_const_iterator{&rows};
}

template <typename T>
typename DynamicMatrix<T>::row_const_iterator DynamicMatrix<T>::end() const
{
...
template <typename T>
T& DynamicMatrix<T>::row_iterator::operator*()
{
    return *current_vec_iter;
}
...
template <typename T>
typename DynamicMatrix<T>::row_iterator DynamicMatrix<T>::begin()
{
    return row_iterator{&rows};
}

template <typename T>
typename DynamicMatrix<T>::row_iterator DynamicMatrix<T>::end()
{
    return row_iterator{&rows.rbegin()->second, &rows};
}
...
template <typename T>
void DynamicMatrix<T>::Resize(const int n_row, const int size)
{
    rows.find(n_row)->second.resize(size);
}
...
template <typename T>
const std::vector<const T*> DynamicMatrix<T>::Column(const int n_col) const
{
...
template <typename T>
const std::vector<std::vector<const T*>> DynamicMatrix<T>::Columns() const
{
    std::vector<std::vector<const T*>> cols;
    for (int i = 0; i < ColumnCount(); ++i)
    {
        cols.push_back(Column(i));
    }
    return cols;
}
...
template <typename T>
int DynamicMatrix<T>::RowCount() const
{
    return n_rows;
}
