...
protected:
    ...
    JobShopProblem genetic_job_shop;
};


TEST_F(GeneticJobshopTest, CrossOverTest)
{
    auto parent1 = genetic_job_shop.GenerateSolution();
    auto parent2 = genetic_job_shop.GenerateSolution();
    std::vector parents{std::move(parent1), std::move(parent2)};
    auto child = genetic_job_shop.CrossOver(parents);
    ASSERT_EQ(child.genotype.RowCount(), 5);
    ASSERT_EQ(child.genotype.Size(), 50);
    ASSERT_EQ(child.genotype.RowSize(0), 10);
    auto row_is_correct = [&child](const int row_idx)
    {
        return std::ranges::all_of(
            child.genotype.Row(row_idx),
